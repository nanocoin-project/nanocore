...
var nanocoreDefaults = require('../config');
...
function PeerManager(config) {
  ...
  this.config = config || {};
  for (var i in nanocoreDefaults)
    if (nanocoreDefaults.hasOwnProperty(i) && this.config[i] === undefined)
      this.config[i] = nanocoreDefaults[i];
